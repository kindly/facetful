...
**Memory-vs-disk decision policy** (deliberately boring): placement is per table at load time — `db.load()` fully resident (default for files under ~64-128 MB, scaled by `navigator.deviceMemory` where available), `db.loadOpfs()` disk-backed with `cacheBytes` (conservative default). No automatic mid-session demotion; "spill this table" would be an explicit one-shot image write + source swap. Aggregation state and results never spill in v1 — bounded by output cardinality, documented; operator spill is the hypothetical M10.
