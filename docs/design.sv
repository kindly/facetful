...
**gratnav's verdict on `"all"`: not for its grid.** Measured in Chromium with the d58 pack, all 1.52M rows held: page buffers 190 → 144 MB, but the worker's high-water 519 → 593 MB (the hash pass's map and the codes) and the full load 0.4–0.5 → 0.7–0.8 s; Chrome's total unchanged. The grid stays on `true`. That is the option doing its job — the trade is visible and the caller declined it — and the reason `"all"` is not the default. A second finding from the same run: its reader called `dictionary()` the moment the result landed, which decoded 1.1M title and recipient strings on the main thread (0.7 s) before showing a row. `dictionary()` now says so in its doc comment; the per-row path (`rows()`, `column()`) decodes one code at a time into a lazily filled cache, and `dictValue(name, code)` exposes that path for readers that touch a few rows of a big dictionary.
...
**Correction, same day (David asked whether the ratio includes aggregation).** It does — the fused time covers masks, per-facet counts and the measure's sum — but the two sides did not compute the same thing: the SQL queries summed the measure *per group* on every facet, while `facet_refresh` counts per group and sums the measure once overall. Like for like (SQL count-only per facet + the totals query, warm): **PUDL 5.1 → 1.4 ms, 3.7×; grantnav 41 → 15 ms, 2.7×.** The per-group sum is a third of a PUDL facet query and 40% of a grantnav one (the f64 lane is 12 MB there, copied once per facet). So the honest ceiling for a fused executor that also sums per group is nearer 3× than 4.5×, and the sum lane is exactly the thing shared loading would stop copying eight times.
