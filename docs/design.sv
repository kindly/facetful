...
**Write-behind adoption (adopted intent for M5 — supersedes the placement heuristic below as the default)**: every table takes one logical path — query immediately from the fetched in-memory buffer, stream it to OPFS in the background, then swap the source to the OPFS handle and drop the buffer. Memory-residency becomes a *transient state*, not a placement decision. Cost of the disk route once adopted: first-touch segment reads only (~10-50 µs/call + hundreds of MB/s ⇒ ~50-150 ms for a cold 7-column query at 1M rows, single-digit ms at 200K, once per session, hideable by `warm()`); steady-state is byte-identical to resident since both scan the same cached buffers. Repeat sessions open from OPFS with no fetch. Safari private mode (no OPFS): the transient state simply persists — which is the memory fallback anyway. M5's first task: measure our real `opfs_read` round-trip so these are numbers, not literature. The budget policy below survives as the cache-sizing rule and the escape hatch.
