...
- **Measured example — regex** (spikes/regex-bench, shipping profile): the `regex` crate is disqualifying for core at **349 KB gz** (256 KB even with unicode stripped — the meta-engine machinery, not the tables, is the bulk); **`regex-lite` costs ~26 KB gz net** (PikeVM, linear-time, no unicode property classes) and is the right engine if `regexp()` ever ships — as a **Tier-2 plug-in module**, costing core zero. Dictionary-aware predicate evaluation makes its slower matching moot on dict columns (pattern runs per dictionary entry, not per row). A ~300-LoC Kernighan-style hand-rolled subset remains the fallback below that.
