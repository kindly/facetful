...
**How the vectorization actually works** (asked precisely, so verified precisely by emitting assembly — `cargo rustc -- --emit=asm` produces textual wasm/native asm to grep): this is *database-style* vectorization — columnar batch kernels in 100 % safe Rust (no `unsafe` in the executor; `unsafe` only at the wasm FFI boundary and in the measurement spikes). **Instruction census of the shipping builds: the opt-3 gains are NOT auto-vectorization.** The wasm has 466 SIMD instructions but 389 are `v128.load/store` (memcpy-style bulk moves) with zero `f64x2`/`i16x8` compute; native shows 10 packed-compute instructions total. The 2-3× came from unrolling, inlining, and better scalar codegen — our hot loops are gather/scatter- and branch-shaped (per-lane validity tests, `sums[gids[i]] += x[i]`, code-table lookups), which LLVM won't vectorize on any target. Consequence: the lang-bench spike's hand-written SIMD headroom (4-10× on scan kernels, `core::arch::wasm32` — mostly safe intrinsics, `unsafe` only for raw loads) is **entirely unclaimed**, and branchless kernel forms (validity as arithmetic masks) would be the prerequisite. That is the named path if like_scan/arith_scan ever need to close their 1.7× gap.
</sv-prose>
